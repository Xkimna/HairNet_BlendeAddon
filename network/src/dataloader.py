...
class Dataloader(object):
    ...
    def __init__(self, data_path, batch_size):
        ...
        self.train_x_dir = os.path.join(data_path, 'train_data\\x_png')
        self.train_y_dir = os.path.join(data_path, 'train_data\\y')
        self.test_x_dir = os.path.join(data_path, 'test_data\\x_png')
        self.test_y_dir = os.path.join(data_path, 'test_data\\y')
        self.pos_mean, self.pos_std, self.curv_mean, self.curv_std = get_mean_std(data_path)
        self.order = list(range(132))       # load test data first
    ...
    def get_test_data(self):
        ...
        test_x = self.load_image(self.test_x_dir, 0, 132)
        test_y = self.load_gt(self.test_y_dir, 0, 132)
